...
export default function Skills() {
  const row = (items: string[], reverse = false) => (
    <div className="flex overflow-hidden border-t border-ink/15 py-6 select-none" aria-hidden={reverse}>
      <div
      ...
      >
        {[...items, ...items].map((s, i) => (
          <span key={`${s}-${i}`} className="flex items-baseline gap-12 whitespace-nowrap">
            <span className="font-display text-2xl md:text-3xl text-ink/70 hover:text-gold transition-colors" style={{ letterSpacing: '-0.02em' }}>
              {s}
            </span>
